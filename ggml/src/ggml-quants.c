...
static inline float q4nx_bf16_bytes_to_f32(const uint8_t * p) {
...
void dequantize_row_q4nx(const block_q4nx * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int e = 0; e < qk; e++) {
            ...
            const int g = c / 32;
            float scale = q4nx_bf16_bytes_to_f32(b->scales + (r*8+g)*2);
            float zp    = q4nx_bf16_bytes_to_f32(b->zeros  + (r*8+g)*2);
