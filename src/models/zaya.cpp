...
void llama_model_zaya::load_arch_hparams(llama_model_loader & ml) {
    ...
    for (uint32_t i = 0; i < hparams.n_layer(); ++i) {
        hparams.is_recr_impl[i] = true;  // every layer has CCA attention
    }
...
void llama_model_zaya::load_arch_tensors(llama_model_loader & ml) {
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        // CCA attention — every layer has both CCA and MoE
        ...
        if (layer.res_scale_hs_mlp == nullptr) {
        ...
        if (layer.res_scale_res_mlp == nullptr) {
        ...
        // Router network — every layer has MoE
...
llama_model_zaya::graph::graph(const llama_model & model, const llm_graph_params & params)
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // Apply residual scaling from the PREVIOUS layer's post-MLP scales.
        // Each Zaya layer does: CCA attention → residual → MoE → residual → residual scaling.
        // The scaling from layer (il-1)'s MoE output is applied at the start of layer il.
        // For il==0, no previous scaling exists — identity pass-through.
        if (il == 0) {
            residual = ggml_cast(ctx0, inpL, GGML_TYPE_F32);
        } else {
            const auto & prev_layer = model.layers[il - 1];
            ggml_tensor * hs = prev_layer.res_scale_hs_mlp   ? prev_layer.res_scale_hs_mlp   : prev_layer.res_scale_hs;
            ggml_tensor * hb = prev_layer.res_scale_hs_mlp_b ? prev_layer.res_scale_hs_mlp_b : prev_layer.res_scale_hs_b;
            ggml_tensor * rs = prev_layer.res_scale_res_mlp  ? prev_layer.res_scale_res_mlp  : prev_layer.res_scale_res;
            ggml_tensor * rb = prev_layer.res_scale_res_mlp_b ? prev_layer.res_scale_res_mlp_b : prev_layer.res_scale_res_b;
        ...
        // Skip connection: add the layer input (residual) back to attention output
        cur = ggml_add(ctx0, cur, residual);
        cb(cur, "attn_skip", il);

        // ── Post-attention norm (if available) ──
        if (layer.attn_norm_2 != nullptr) {
            cur = build_norm(cur, layer.attn_norm_2, nullptr, LLM_NORM_RMS, il);
            cb(cur, "post_attn_norm", il);
        }

        // ===== MoE (every layer) =====
        router_h = ggml_mul_mat(ctx0, layer.zaya_router_down, cur);
        ...
        // Skip connection: add the layer input (residual) back to MoE output
        cur = ggml_add(ctx0, cur, residual);
        cb(cur, "moe_skip", il);
